...
// ANTI LINK GRUP
	    ...
	    if (authorname != undefined) { } else { authorname = groupName }				function addMetadata(packname, author) {	
				...
				if (!packname) packname = 'MAHLI'; if (!author) author = 'FAISAL WAPI';	
...
*/
			...
			break
				...
				nul = body.slice(7)
				reply('「❗」WAIT YAA *BOT-YOE* NULIS DULU')
...
*/
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							baby.sendMessage(_.jid, buff, image, {caption: `*「 IKLAN/PENGUMUMAN BOT-YOE 」*\n\n${body.slice(4)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `*「「 IKLAN/PENGUMUMAN BOT-YOE 」」*\n\n${body.slice(4)}`)
						}
						reply('*「 SUKSES BOSKU 」*')
					}
					break
